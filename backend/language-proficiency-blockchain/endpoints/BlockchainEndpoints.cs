...
public class BlockchainEndpoints : IEndpoint
{
    ...
    /// </returns>
    public static async Task<Results<Ok<Node>, BadRequest<string>>> ProposeNode(
        [FromServices] BlockchainService chain,
        [FromBody] ProposeNodeRequest req)
    {
        try
        {
            var node = new Node { PublicKeyPem = req.PublicKeyPem, Address = req.Address };
    ...
    public static async Task<Ok<List<Node>>> ListNodes([FromServices] AppDbContext db)
    {
