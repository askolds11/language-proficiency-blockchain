...
const props = defineProps({
  ...
  mode: { type: String, default: 'list' }, // list || toggle
...
function onDetect(detectedCodes) {
  ...
  if (!props.showAlerts && props.kind === 'single') {
    pause.value = true;
    nextTick(() => {
      pause.value = false;
    });
...
<template>
  <div class="lx-qr-scanner-wrapper" :class="{ 'drag-over': dragOver }">
    <div class="lx-toolbar" v-if="camerasList?.length > 1 || fileUploader || flashlight">
      <LxToggle
        v-model="torch"
        v-if="!torchNotSupported && flashlight"
      ...
      />
      <LxButton
        v-if="camerasList?.length > 1 && mode === 'toggle'"
        icon="camera"
        kind="ghost"
        @click="switchCamera()"
      />
      <LxDropDownMenu v-if="camerasList?.length > 1 && mode === 'list'">
    ...
    <div class="lx-qr-scanner">
      <Transition name="fade">
        <qrcode-stream
          ...
          :paused="kind === 'single' ? error || accepted || pause : false"
          @camera-on="cameraOn"
          v-if="showPreview"
        >
